...
for (int i = 0; i < t; i++) {
    ...
    cin>>n;
  int count = 0;
  // Loop through each number from 1 to n[i]
  for (int j = 1; j <= n; j++) {
    // If the number of digits in j is 1, then j is extremely round
    if (numberOfDigits(j) == 1) {
      count++;
    }
  }
  // Print the number of extremely round integers between 1 and n[i]
  cout << count << endl;
}
}