...
for(ll i=1;i<=n;i++)
{
    ll x=min(left[i],right[i]);
    left[i]-=x;
    right[i]-=x;
}

l = accumulate(left.begin(),left.end(),0LL);
r = accumulate(right.begin(),right.end(),0LL);
...
ll transfer = abs(r-l)/2;

ll ans = 2LL*transfer+l;

for(ll i=1;i<=n;i++)
{

    while(transfer > 0 && right[i]>1)
    {
        transfer--;
        ans--;
        right[i]-=2;
