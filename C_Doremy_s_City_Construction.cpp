...
for(ll i=1;i<=n;i++)
{
    temp[i] = kmap[arr[i]].first;


    temp1[i] = kmap[arr[i]].second;
}

for(ll j = 1;j<=n;j++)
{

    res = max(res,(temp[j]-1)*(n - temp1[j]) + (temp1[j] - temp[j] + 1)*(temp[j]-1));
}
